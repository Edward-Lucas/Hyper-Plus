pushd "%~dp0"
set "folderPath=C:\Users\%username%\Videos\Hyper+"
if not exist "%folderPath%" (
    mkdir "%folderPath%"
    echo 폴더가 생성되었습니다: %folderPath%
) else (
    echo 폴더가 이미 존재합니다: %folderPath%
)
...
scrcpy --no-playback --record C:\Users\%username%\Videos\Hyper+\%filename%-hyper+record.mkv --window-title="Close Window, Stop Record" -e
pause